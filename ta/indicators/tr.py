...
class TR(Indicator):
    ...
    def calc(self, context):
        ...
        return Series(talib.TRANGE(low_series.values, high_series.values, close_series.values),
...
class ATR(TR):
    ...
    def calc(self, context):
        ...
        high_series = context.get_data(self.high_data_id)
        close_series = context.get_data(self.close_data_id)
        return Series(talib.ATR(low_series.values, high_series.values, close_series.values, self.period),
                      index=low_series.index.values)
...
class NATR(ATR):
    def calc(self, context):
        ...
        high_series = context.get_data(self.high_data_id)
        close_series = context.get_data(self.close_data_id)
        return Series(talib.NATR(low_series.values, high_series.values, close_series.values, self.period),
                      index=low_series.index.values)
